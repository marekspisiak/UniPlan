...
const EventForm = ({
  ...
  const validateForm = () => {
    ...
    if (form.repeat) {
      ...
      if (!form.repeatUntil)
        return "Pri opakovaní eventu musíš zadať do kedy sa má opakovať.";
      if (form.startTime >= form.endTime)
        return "Čas začiatku musí byť pred časom konca.";
      if (form.repeatUntil < form.startDate)
  ...
  return (
    <div className={styles.eventForm}>
      ...
      <Form onSubmit={handleSubmit}>
        ...
        <Form.Group className="mb-3">
          <Form.Label>
            {form.repeat ? "Začiatok opakovania" : "Dátum"}
          </Form.Label>
          <Form.Control
            type="date"
            name="startDate"
        ...
        <ImageUploader
          ...
          multiple={false}
          existing={initialData.mainImage ? [`${initialData.mainImage}`] : []}
        />
        ...
        <ImageUploader
          ...
          multiple
          max={5}
          existing={initialData.gallery?.map((g) => `${g.url}`) || []}
        />
